...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!args.length)
      return message
        .reply(`:x: Usage: ${message.client.prefix}play <YouTube URL | Video Name | Soundcloud URL>`)
        .catch(console.error);
      ...
      try {
        https.get(url, function (res) {
          if (res.statusCode == "302") {
            ...
            return message.reply(":x: No content could be found at that url.").catch(console.error);
          }
        });
      } catch (error) {
        console.error(error);
    ...
    if (serverQueue) {
      ...
      has been added to the queue by ${message.author}`)
        .catch(console.error);
    }
