...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!args.length)
      return message
        .reply(`:x: Usage: ${message.client.prefix}playlist <YouTube Playlist URL | Playlist Name>`)
        .catch(console.error);
    ...
    if (serverQueue && channel !== message.guild.me.voice.channel)
      return message.reply(`:x: You must be in the same channel as ${message.client.user}`).catch(console.error);
    ...
    if (urlValid) {
      try {
        ...
        console.error(error);
        return message.reply(":x: Playlist not found :(").catch(console.error);
      }
    ...
    if (!serverQueue) {
      ...
      try {
        ...
        await channel.leave();
        return message.channel.send(`:x: Could not join the channel: ${error.message}`).catch(console.error);
      }
    }
  }
};
